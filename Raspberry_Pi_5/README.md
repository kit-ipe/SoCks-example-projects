# SoCks Example Project for the Raspberry Pi 5
...
2. Move to this directory.
    ```
    $ cd SoCks-example-projects/Raspberry_Pi_5
    ```
...
2. Write the image to an SD card. Identify the device you want to write to e.g. with `lsblk` and unmount it.
    ...
    **WARNING:** Before writing to a device, first identify it, e.g. with `lsblk`, and unmount it.
    ```
    $ sudo dd if=rpi5-example-project_sd_card.img of=/dev/sdX bs=1M status=progress
    ```
...
1. Insert the SD card into your Raspberry Pi 5 and connect a display via micro HDMI.
